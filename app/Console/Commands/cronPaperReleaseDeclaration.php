...
use Mail;
class cronPaperReleaseDeclaration extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'date:resultDeclaration';
    ...
    public function handle()
    {
        ...
        $paperResult =  PackagePaper::where(function($query) use($date) {
                            $query->whereDate('result_declaration_date', '=', $date);
        ...
        foreach($paperResult as $value)
        {
            $subject = trans('language.mail_subject');
            $value['package_name'] = $value['packages']['name'];
            $value['paper_name']   = $value['papers']['name'];
            // Send Mail
            Mail::send('emails.admin-panel.paper-result-declare-email', $value, function ($message) use ($value, $subject) {
                $message->to($value['email'], $value['name'])->subject($subject);
            });

            // Push Notification
            $title = trans('language.mail_subject');
            $notification_type = 2;  // Paper Result Declare
            $message = 'Your Paper'. $paper_name.' result has been Declaration Today!';
